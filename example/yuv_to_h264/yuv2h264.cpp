...
const char* inFileNameDefault = "./yuv420p.yuv";
const char* outFileNameDefault = "./encode_yuv420p.h264";

#define FILE_WIDTH      1920
#define FILE_HEIGHT     1080
#define FILE_FIX_FMT    AV_PIX_FMT_YUV420P
...
int main(int argc, char* argv[])
{
    ...
    {
        inFileName = (char *)inFileNameDefault;
    }
    if (outFileName == nullptr)
    {
        outFileName = (char *)outFileNameDefault;
    }
    ...
    //查找指定编码器
    encoder = (AVCodec *)avcodec_find_encoder_by_name(AVCODEC_ENCODER_NAME);
    if (!encoder) {
        printf("Decoder not found, try another\n");
    ...
    printf("Find encoder : %s\n", encoder->name);

    //申请编码器上下文
    codecContent = avcodec_alloc_context3(encoder);
    ...
    //必设参数
    codecContent->width = FILE_WIDTH;
    codecContent->height = FILE_HEIGHT;
    codecContent->time_base = AVRational{ 1, 25 };
    codecContent->pix_fmt = FILE_FIX_FMT;
    codecContent->gop_size = 60; //关键帧间隔，默认250
    codecContent->framerate = AVRational{ 25, 1 };

    //初始化编码器上下文
    ret = avcodec_open2(codecContent, encoder, NULL);
    if (ret < 0) {
        fprintf(stderr, "Could not open encoder: %d\n", ret);
        exit(1);
    }
